...
> compositor policies, and shell widgets are completed through strict mode-specific builders. The
...
> atomic libdrm/GBM KMS presentation. Compositor policy, default window frames, pointers, semantic
...
| Retained render scene | Operational | Dense box/glyph/image/material instances, ordered draw items, clip/spatial tables, scene deltas, dirty ranges, damage, glyph-atlas pages, versioned RGBA image writes/removals, and built-in material resources execute against the software path and feed the Vulkan backend |
| Software renderer | Operational, limited | `telorgon-renderer-software` is the CPU reference implementation. It rasterizes ordered boxes, antialiased rounded asymmetric border rings, outside focus outlines, two shadows, primitive-local opacity, glyph masks, RGBA images, materials, affine spatial transforms, rounded clips, and opaque/alpha batches with retained damage and explicit readback. Broader effects and production color-management qualification remain incomplete |
...
| Linux Wayland desktop host | Operational, limited | With `desktop-wayland-linux`, `Application::desktop_environment(...)` owns one Linux-only thread that combines official `libwayland-server` transport, Telorgon protocol state, libseat/libinput/XKB pointer/keyboard/touch delivery, composed policy/frame/pointer/icon/widget layers, transformed/scaled/viewporter SHM client images, data-device selection and pointer/touch drag-and-drop with MIME FD/action negotiation, activation, session lock, constraints, interactive window management, commit-scoped presentation feedback, triple-buffered GBM primary scanout, completion-retired atomic ARGB8888 cursor-plane commits with coalesced cursor-only motion and software fallback, owned Vulkan rendering or mapped software fallback, and atomic libdrm KMS. Compile and portable state tests pass without launching the host. The path is single-output, has no Linux hardware/conformance run, and does not yet provide direct per-surface managed Vulkan composition, hotplug/session reconstruction, input methods, DMA-BUF feedback v4/v5, DRM syncobj, or the rest of the unadvertised extension set |
...
performs explicit foreign ownership transfer, and exports release synchronization once. Its ignored
