...
impl Environment {
    pub(crate) fn inherited() -> Self { Self(std::env::vars_os().collect()) }
    ...
    pub(crate) fn clear_transient(&mut self) {
        for key in ["WAYLAND_SOCKET", "XDG_ACTIVATION_TOKEN", "DESKTOP_STARTUP_ID"] { self.0.remove(OsStr::new(key)); }
    }
    ...
    pub(crate) fn desktop(&self, runtime: &Path, socket: &str, identity: &str) -> Self {
        ...
        env.0.insert("XDG_RUNTIME_DIR".into(), runtime.as_os_str().into());
        for (key, value) in [("WAYLAND_DISPLAY", socket), ("XDG_SESSION_TYPE", "wayland"),
            ("XDG_CURRENT_DESKTOP", identity), ("XDG_SESSION_DESKTOP", identity)] {
        ...
        if env.get("DBUS_SESSION_BUS_ADDRESS").is_none() && runtime.join("bus").exists() {
            ...
            #[cfg(unix)]
            {
                ...
                let encoded: String = path.as_os_str().as_bytes().iter().map(|b| {
                    if b.is_ascii_alphanumeric() || b"/-_.".contains(b) { char::from(*b).to_string() }
                    else { format!("%{b:02x}") }
                }).collect();
                env.0.insert("DBUS_SESSION_BUS_ADDRESS".into(), format!("unix:path={encoded}").into());
            }
        }
        env
    }
    ...
    pub(crate) fn get(&self, key: &str) -> Option<&OsStr> {
        self.0.get(OsStr::new(key)).filter(|value| !value.is_empty()).map(OsString::as_os_str)
    }

    #[cfg(target_os = "linux")]
    pub(crate) fn runtime_directory(&self) -> Result<PathBuf> {
        let path = self.get("XDG_RUNTIME_DIR").map(PathBuf::from).unwrap_or_else(|| {
...
fn validate_runtime(path: &Path) -> Result<()> {
    ...
    let metadata = std::fs::symlink_metadata(path).map_err(|e| Error::Invalid(format!(
        "runtime directory {} is unavailable ({e}); start from a normal local PAM login", path.display())))?;
