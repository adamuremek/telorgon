...
fn main() {
    ...
    let source_dir = manifest_dir.join("shaders");
    let output_dir =
        PathBuf::from(env::var_os("OUT_DIR").expect("out dir")).join("renderer_builtins");
    ...
    let shaders = [
        (
            "quad.vert.glsl",
            "quad.vert.spv",
            ShaderCompilerStage::Vertex,
        ),
        (
            "solid.frag.glsl",
            "solid.frag.spv",
            ShaderCompilerStage::Fragment,
        ),
        (
            "glyph.vert.glsl",
            "glyph.vert.spv",
    ...
    for (source_name, output_name, stage) in shaders {
        let source_path = source_dir.join(source_name);
        let output_path = output_dir.join(output_name);
        println!("cargo:rerun-if-changed={}", source_path.display());
        compile_shader(&compiler, stage, &source_path, &output_path);
    }
...
fn find_shader_compiler() -> Option<ShaderCompiler> {
    ...
    if let Some(vulkan_sdk) = env::var_os("VULKAN_SDK") {
        ...
        candidates.push(ShaderCompiler {
            ...
            flavor: CompilerFlavor::Glslc,
        });
        candidates.push(ShaderCompiler {
            path: sdk_bin.join(executable_name("glslangValidator")),
            flavor: CompilerFlavor::GlslangValidator,
        });
    }
...
fn compile_shader(
    ...
    stage: ShaderCompilerStage,
    source_path: &Path,
    output_path: &Path,
) {
    let mut command = Command::new(&compiler.path);
    match compiler.flavor {
        CompilerFlavor::Glslc => {
            let stage_name = shader_stage_name(stage);
            command
                .arg(format!("-fshader-stage={stage_name}"))
                .arg(source_path)
                .arg("-o")
                .arg(output_path);
        }
        CompilerFlavor::GlslangValidator => {
            command
                .arg("-V")
                .arg("-S")
                .arg(shader_stage_name(stage))
                .arg("-o")
                .arg(output_path)
                .arg(source_path);
        }
...
fn shader_stage_name(stage: ShaderCompilerStage) -> &'static str {
    match stage {
        ShaderCompilerStage::Vertex => "vert",
        ShaderCompilerStage::Fragment => "frag",
    }
}
